...
_guess_ssh_auth_sock() {
    ...
    # Collect candidates: agent sockets left by sshd forwarding or ssh-agent.
    # Typical paths:
    #   /tmp/ssh-*/agent.*              (ssh-agent / OpenSSH < 8.x)
    #   /tmp/ssh-*/S.ssh                (some distributions)
    #   /run/user/<uid>/openssh_agent   (systemd socket activation)
    ...
    if [[ ${#candidates[@]} -eq 0 ]]; then
        ...
        while [[ "$pid" -gt 1 ]]; do
            ...
            if [[ "$cmdline" == sshd* ]]; then
                candidate=$(grep -oP '(?<=SSH_AUTH_SOCK=)[^\0]+' \
    ...
    for candidate in "${candidates[@]}"; do
        if SSH_AUTH_SOCK="$candidate" ssh-add -l &>/dev/null; then
            found="$candidate"
            break
        fi
    done
